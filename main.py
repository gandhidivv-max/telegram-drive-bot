...
def get_all_user_boards(access_token):
    ...
    while True:
        url = "https://api.pinterest.com/v5/boards?page_size=25"
        if bookmark:
...
def post_media_to_telegram(media_url, is_video, is_special_board, board_name):
    ...
    # Telegram API కి అప్‌లోడ్ అయ్యే సమయంలో నోటిఫికేషన్ ఎర్రర్ రాకుండా Retry & Timeout మెరుగుపరచడం
    max_retries = 3
    for attempt in range(max_retries):
        try:
            ...
            logging.warning(f"Telegram Attempt {attempt+1}/{max_retries} failed: {err_text}")
            time.sleep(2)
        except Exception as e:
            logging.warning(f"Telegram Exception Attempt {attempt+1}/{max_retries}: {e}")
            time.sleep(2)

    # 3 సార్లు ప్రయత్నించిన తర్వాత కూడా ఫెయిల్ అయితే కేవలం లాగ్స్‌లో చూపిస్తుంది (అనవసరమైన టెలిగ్రామ్ మెసేజ్‌లు పంపదు)
    logging.error(f"Telegram Posting Failed after {max_retries} retries for URL: {media_url}")
    return False
